...
# → install 顺序回归。退出码非零 = 存在失败；SKIPPED 不算失败但必须显式输出，不得声称通过。
set -uo pipefail
cd "$(dirname "$0")/.."

rc=0
echo "==== [1/5] 语法检查 (tests/lint.sh) ===="
bash tests/lint.sh || rc=1

echo "==== [2/5] 函数清单断言 (tests/functions.sh) ===="
bash tests/functions.sh || rc=1

echo "==== [3/5] CLI 冒烟 (tests/smoke.sh) ===="
bash tests/smoke.sh || rc=1

echo "==== [4/5] offline-first 行为验证 (tests/offline-first.sh) ===="
bash tests/offline-first.sh || rc=1

echo "==== [5/5] install 顺序回归 (tests/install-order.sh) ===="
