...
【原型已验证】终定稿 openPhpExtensionsModal 为本模型。
...
约束：**Go 引擎**禁止返回自然语言错误信息给前端；引擎日志（任务抽屉内）可保留英文技术术语。【C2·阶段 0 豁免】bash 桥阶段引擎输出为中文自然语言日志，状态推断走标记集契约（设计文档 §12.10），不受本条约束。
