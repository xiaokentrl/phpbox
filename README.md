...
只要 `offline/php/<版本>/` 里有已验证的 APK 闭包和 PECL 包（上次成功构建自动晋升），断网也能完整重装：
...
| 命令 | 说明 |
| --- | --- |
| `phpbox mysql install <版本> [--port 端口]` | 安装 MySQL，完成后显示 root 密码（存入 `.env`） |
...
| `offline/`、`cache/go/` | 构建验证成功后自动晋升 / Go 命令 |
...
As long as `offline/php/<version>/` holds verified APK closures and PECL tarballs (promoted automatically after each successful build), a full reinstall works with no network:
...
| Command | Description |
| --- | --- |
| `phpbox mysql install <version> [--port N]` | Install MySQL; root password shown and saved to `.env` |
...
├── offline/                      # offline build cache (php/<version>/apk/, pecl/)
...
| `offline/`, `cache/go/` | promoted after verified builds / Go commands |
